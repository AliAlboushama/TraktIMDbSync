...
def _wait_for_new_csv_download(directory, known_files, max_wait=30):
    ...
    while time.time() - download_wait_start < max_wait:
        current_files = {
    ...
    new_files = sorted(current_files - known_files, key=os.path.getmtime, reverse=True)
...
def generate_imdb_exports(
    ...
    print(
        "    - Waiting for IMDB to generate exports (this may take a few minutes)...",
        flush=True,
    )
    ...
    while time.time() - start_time < max_wait_time:
        ...
        try:
            ...
            print(
                "      No items found when attempting to generate IMDB exports. Assuming no IMDB watchlist, ratings or check-ins to download.",
                flush=True,
            )
            break
        ...
        if not in_progress_items:
            print(
                f"      OK All exports ready! (took {elapsed_time} seconds)", flush=True
            )
            break
        else:
            ...
            if elapsed_time - last_status_time >= status_interval:
                ...
                progress_items = ", ".join(
                    [item.split("\n")[0] for item in in_progress_items[:2]]
                )  # Show first 2 items
                if len(in_progress_items) > 2:
                    progress_items += f" and {len(in_progress_items) - 2} more"
                print(
                    f"      Waiting... ({elapsed_time}s elapsed, max {remaining_minutes}m remaining) - Processing: {progress_items}",
                    flush=True,
                )
...
def download_imdb_exports(
    ...
    try:
        ...
        print(
            "      No items found when attempting to download IMDB exports. Assuming no IMDB watchlist, ratings or check-ins to download.",
            flush=True,
        )
        return driver, wait
        ...
        for item in summary_items:
            if item_text.lower() in item.text.lower():
                try:
                    ...
                    print(
                        f"      Export button not found for '{item_text}' on IMDB exports page https://www.imdb.com/exports/",
                        flush=True,
                    )
                    return None
        print(
            f"      No section found matching '{item_text}' on IMDB exports page https://www.imdb.com/exports/",
            flush=True,
        )
        return None
        ...
        if csv_link:
            try:
                ...
                if downloaded_file:
                    ...
                    print(
                        f"      OK {display_name} downloaded successfully", flush=True
                    )
                else:
                    print(
                        f"      Failed to locate downloaded file for {display_name.lower()}",
                        flush=True,
                    )
            except Exception as e:
                print(
                    f"      Failed to download {display_name.lower()}: {str(e)}",
                    flush=True,
                )
        else:
            if sync_watchlist_value and file_name == "watchlist.csv":
                print(
                    f"      WARN No export available for {display_name.lower()}, skipping...",
                    flush=True,
                )
            elif (
                sync_ratings_value or mark_rated_as_watched_value
            ) and file_name == "ratings.csv":
                print(
                    f"      WARN No export available for {display_name.lower()}, skipping...",
                    flush=True,
                )
    ...
    if downloads_count > 0:
        print(f"      OK Download complete ({downloads_count} file(s))", flush=True)
    ...
    try:
        ...
        if not os.path.exists(watchlist_path):
            raise FileNotFoundError(
                f"IMDB watchlist data not found. '{watchlist_filename}' not found in the directory"
            )
        ...
        with open(watchlist_path, "r", encoding="utf-8") as file:
            ...
            if missing_columns:
                raise ValueError(
                    f"Required columns missing from CSV file: {', '.join(missing_columns)}"
                )
        ...
        imdb_watchlist_size = 0
        error_message = (
            f"{watchlist_filename} file not found. Assuming the IMDB list is empty."
        )
        print(error_message)
...
def get_imdb_ratings(driver, wait, directory):
    ...
    try:
        ...
        if not os.path.exists(ratings_path):
            raise FileNotFoundError(
                f"IMDB ratings data not found. '{ratings_filename}' not found in the directory"
            )
...
def get_imdb_checkins(driver, wait, directory):
    ...
    try:
        ...
        if not os.path.exists(checkins_path):
            raise FileNotFoundError(
                f"IMDB Check-ins data not found. '{checkins_filename}' not found in the directory"
            )
        ...
        with open(checkins_path, "r", encoding="utf-8") as file:
            ...
            date_column = "Created" if "Created" in header_index else "Date Rated"
            required_columns = ["Title", "Year", "Const", date_column, "Title Type"]
            missing_columns = [
                col for col in required_columns if col not in header_index
            ]
            if missing_columns:
            ...
            for row in reader:
                ...
                year = int(year) if year else None
                imdb_id = row[header_index["Const"]]
                date_added = row[header_index[date_column]]
                media_type = row[header_index["Title Type"]]
                # Convert date format
                date_added = datetime.strptime(date_added, "%Y-%m-%d").strftime(
                    "%Y-%m-%dT%H:%M:%S.000Z"
...
def get_imdb_reviews(driver, wait, directory):
    ...
    # Load page
    success, status_code, url, driver, wait = EH.get_page_with_retries(
        "https://www.imdb.com/profile", driver, wait, total_wait_time=60
    )
    if not success:
        # Page failed to load, log warning and return empty reviews
        print(
            "      WARN Could not load IMDB profile page for reviews. Skipping reviews sync."
        )
        EL.logger.warning(
            f"Failed to load IMDB profile page. Status code: {status_code}. URL: {url}"
        )
    ...
    try:
        ...
        try:
            ...
            print(
                "      WARN IMDB profile page did not redirect to user page. Skipping reviews sync."
            )
            ...
            try:
                try:
                    review_elements = wait.until(
                        EC.presence_of_all_elements_located(
                            (By.CSS_SELECTOR, "div[data-testid='review-card-parent']")
                        )
                    )
                except (NoSuchElementException, TimeoutException):
                ...
                for element in review_elements:
                    ...
                    review["Title"] = element.find_element(
                        By.CSS_SELECTOR,
                        "div[data-testid='review-title-header'] h3 span",
                    ).text.strip()
                    date_text = element.find_element(
                        By.CSS_SELECTOR, "li.review-date"
                    ).text
                    review["Year"] = (
                        int(date_text.strip().split()[-1]) if date_text else None
                    )
                    review["IMDB_ID"] = (
                        element.find_element(
                            By.CSS_SELECTOR, "div[data-testid='review-summary'] a"
                        )
                        .get_attribute("href")
                        .split("/")[4]
                    )
                    ...
                    spoiler_warning_elements = element.find_elements(
                        By.CSS_SELECTOR, ".review-spoiler-button"
                    )
                ...
                try:
                    ...
                    next_link = driver.find_element(
                        By.CSS_SELECTOR, "div[data-testid='index-pagination-nxt']"
                    )
