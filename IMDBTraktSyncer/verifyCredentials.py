...
def get_credentials_file_path():
    return os.path.join(os.path.abspath(os.path.dirname(__file__)), "credentials.txt")
...
def save_credentials(credentials):
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(credentials, f, indent=4, separators=(", ", ": "))
...
def prompt_get_credentials():
    ...
    if should_refresh:
        ...
        if refresh_token != "empty":
            access_token, refresh_token, expiration_time = authTrakt.authenticate(
...
def prompt_sync_watch_history():
    ...
    while True:
        ...
        print(
            "See FAQ: https://help.imdb.com/article/imdb/track-movies-tv/check-ins-faq/GG59ELYW45FMC7J3"
        )
        user_input = (
...
def prompt_remove_watched_from_watchlists():
    ...
    while True:
        ...
        print(
            "Shows are removed when at least 80% of the episodes are watched AND the series is marked as ended or cancelled."
        )
...
def prompt_remove_watchlist_items_older_than_x_days():
    ...
    while True:
        ...
        print(
            "This setting is meant to help address the 100 item limit in Trakt watchlists for free tier users."
        )
        print(
            "In order to prevent old items from being re-added, it is recommended to disable Trakt watchlist sync"
        )
        print(
            "in other projects when enabling this setting. Such as Reelgood and other similar apps."
        )
        print(
            "If you use the PlexTraktSync project, it is recommended to disable watchlist sync from Plex to Trakt."
        )
        print(
