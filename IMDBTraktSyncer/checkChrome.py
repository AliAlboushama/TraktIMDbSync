...
def get_user_data_directory():
    ...
    # Keep the persistent profile outside the Chrome binary directory.
    # This avoids crashes caused by stale/corrupted state inside the unpacked browser folder.
    user_data_directory = Path(directory) / "ChromeProfile" / version
    ...
    while retries <= max_retries:
        try:
            ...
            if expected_file_size and actual_file_size != expected_file_size:
                raise RuntimeError(
                    f" - Downloaded file size mismatch: expected {expected_file_size} bytes, got {actual_file_size} bytes"
                )
            ...
            if retries > max_retries:
                raise RuntimeError(
                    f" - Failed to download or extract Chromedriver version {version} after {max_retries} retries: {e}"
                )
            ...
            if retries > max_retries:
                raise RuntimeError(
                    f" - Failed to download or extract Chromedriver version {version} due to an unexpected error after {max_retries} retries: {e}"
                )
            time.sleep(wait_interval)  # Wait before retrying
            ...
            try:
                ...
                print(
                    f" - Permission denied when trying to delete {zip_path}. Ensure no other process is using it."
                )
            except Exception as e:
                ...
                for sub_dir in version_dir.iterdir():
                    if sub_dir.is_dir():
                        ...
                        if headless_shell_path.exists():
                            print(
                                f"chrome-headless-shell found in {headless_shell_path}, but script is set to use regular Chrome. Removing entire contents of {chrome_dir}..."
                            )
                ...
                for sub_dir in version_dir.iterdir():
                    if sub_dir.is_dir():
                        ...
                        if chrome_path.exists():
                            print(
                                f"Chrome found in {chrome_path}, but script is set to use chrome-headless-shell. Removing entire contents of {chrome_dir}..."
                            )
                            try_remove(version_dir)
                            return  # Exit the function after removal
